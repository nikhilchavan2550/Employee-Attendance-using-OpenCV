...
def list_folder_names():
    # Path to the 'faces' directory within the 'static' directory
    faces_dir = os.path.join("static", "faces")

    # Get the list of all folders in the 'faces' directory
    folder_names = [
        folder
...
def add_attendance(name):
    ...
    if str(userid) not in list(df["Roll"]):
        print("this user was not in attendence")
        with open(f"Attendance/Attendance-{datetoday}.csv", "a") as f:
            f.write(f"\n{username},{userid},{current_time}")
    else:
        print(
            "this user has already marked attendence for the day , but still i am marking it "
        )
        # with open(f'Attendance/Attendance-{datetoday}.csv','a') as f:
        #     f.write(f'\n{username},{userid},{current_time}')
...
def home():
    ...
    print("below is rednering info of attendence")
    print(names, rolls, times, l)
    userlistt = list_folder_names()
    print(userlistt)
...
def start():
    ...
    while True:
        ...
        cv2.putText(
            frame, "hello", (30, 30), cv2.FONT_HERSHEY_COMPLEX, 2, (255, 255, 255)
        )
